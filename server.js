...
app.get('/search', async (req, res) => {
  try {
    ...
    if(!checkRate(ip)) return res.status(429).json({ error: 'Too many requests. Slow down.' });
    ...
    const url = `https://www.bing.com/images/search?q=${encoded}&safeSearch=${safe}`;
    ...
    if(results.length === 0){
      $('img').each((i, el) => {
        ...
        if(src && !src.startsWith('data:')) results.push({ src, thumb: src, page: null, title: null });
      });
    }
